...
Node ≥ 22.17, npm ≥ 10.9, RAM ≥ 2 GB (4 rec.), 5 GB de disco. Móvil solo en dispositivo físico (los emuladores no funcionan). Cuota de caché de modelos: 512 MiB en React Native, 4 GiB en el resto. `qvac doctor` valida el host (en el Mac ya pasa todo: Metal, adb, Xcode, ffmpeg, Bare, Bun).
...
`finetune({ modelId, dataset (JSONL messages), numberOfEpochs, learningRate, loraModules, checkpointSaveDir, validation, assistantLossOnly })` → adapter `.gguf` → `modelConfig.lora`. Solo arquitecturas `qwen3`, `gemma3`, `bitnet` en F32/F16/Q4_0/Q8_0/TQ*. Corre en móvil (Vulkan/Metal). Idea de alto impacto en Technical si sobra tiempo: LoRA de Qwen3 con 200 ejemplos sintéticos de observaciones de hospital → JSON, entrenado en el propio dispositivo. Muy vistoso, pero es de las últimas horas, no de las primeras.
...
8. Cuota 512 MiB de caché en React Native: en móvil, modelos chicos o delegación.

...
- Dataset: 52 ejemplos sintéticos (8 marcadores de laboratorio × 3 estados × 5 fraseos) → JSON de triaje; 8 de validación. `make-dataset.mjs`.
...
1. El pipeline funciona de punta a punta; el costo es ~30 min por época en el Mac. Para el video: entrenar 1–2 épocas con un dataset de 150–300 ejemplos, y mostrar antes/después.
...
Reproducir: `cd spikes/lora-medpsy && node make-dataset.mjs && node spike.mjs` (requiere `@qvac/sdk@0.18.2` instalado en un `package.json` vecino; en este repo, usar el de `nodo/` o el del curso).
